...
export class KpiService {
  ...
  // ==================== SCORE SANTE GLOBAL ENTREPRISE ====================

  async getHealthScore(filter: PeriodeFilterDto) {
    const latest = await this.getLatestKpis();
    if (!latest.length) {
    ...
    // 1. Score Commercial (25%): CA realisation + variation
    const scoreCommercial = Math.min(100, moyennes.ca_realisation_pct);

    // 2. Score Rentabilite (25%): Taux de marge (objectif 20%)
    const scoreRentabilite = Math.min(100, (moyennes.taux_marge / 20) * 100);

    // 3. Score Tresorerie (25%): Tresorerie positive = 100, DSO < 45j bonus
    let scoreTresorerie = totaux.tresorerie_nette > 0 ? 70 : 30;
    if (moyennes.dso_jours > 0 && moyennes.dso_jours <= 45) scoreTresorerie += 30;
    else if (moyennes.dso_jours > 45 && moyennes.dso_jours <= 60) scoreTresorerie += 15;

    // 4. Score Operationnel (25%): Affaires en cours vs retards/depassements
    ...
    // Status
    let status: 'EXCELLENT' | 'BON' | 'ATTENTION' | 'CRITIQUE';
    if (scoreGlobal >= 80) status = 'EXCELLENT';
    else if (scoreGlobal >= 60) status = 'BON';
    else if (scoreGlobal >= 40) status = 'ATTENTION';
    else status = 'CRITIQUE';

    // Alertes
    const alertes: string[] = [];
    if (moyennes.ca_realisation_pct < 80) alertes.push('CA en dessous des objectifs');
    if (moyennes.taux_marge < 15) alertes.push('Marge insuffisante');
    if (totaux.tresorerie_nette < 0) alertes.push('Tresorerie negative');
    if (moyennes.dso_jours > 60) alertes.push('Delai paiement clients trop long');
    if (tauxRetard > 20) alertes.push('Trop d\'affaires en retard ou depassement');
  ...
  // ==================== ANALYSE DSO/DPO & RECOMMANDATIONS BFR ====================

  async getDsoDpoAnalysis(filter: PeriodeFilterDto) {
    // Recuperer l'historique des KPIs pour analyser la tendance
    const queryBuilder = this.kpiRepository
      .createQueryBuilder('k')
      .select([
        'k.annee',
    ...
    // Calcul des tendances (3 derniers mois vs 3 mois precedents)
    ...
    // Scoring DSO (objectif < 45 jours)
    let scoreDso: number;
    if (dsoActuel <= 30) scoreDso = 100;
    else if (dsoActuel <= 45) scoreDso = 80;
    else if (dsoActuel <= 60) scoreDso = 60;
    else if (dsoActuel <= 90) scoreDso = 40;
    else scoreDso = 20;

    // Scoring DPO (objectif 30-60 jours - equilibre)
    let scoreDpo: number;
    if (dpoActuel >= 45 && dpoActuel <= 60) scoreDpo = 100;
    else if (dpoActuel >= 30 && dpoActuel < 45) scoreDpo = 80;
    else if (dpoActuel > 60 && dpoActuel <= 90) scoreDpo = 70;
    else if (dpoActuel < 30) scoreDpo = 50; // On paie trop vite
    else scoreDpo = 40;

    // Cycle de conversion de tresorerie (CCC)
    const ccc = dsoActuel - dpoActuel;

    // Recommandations BFR
    const recommandations: { type: string; priorite: 'haute' | 'moyenne' | 'basse'; description: string; impact_estime?: number }[] = [];
    ...
    // Status global
    let status: 'OPTIMAL' | 'BON' | 'A_AMELIORER' | 'CRITIQUE';
    const scoreGlobal = (scoreDso + scoreDpo) / 2;
    if (scoreGlobal >= 80) status = 'OPTIMAL';
    else if (scoreGlobal >= 60) status = 'BON';
    else if (scoreGlobal >= 40) status = 'A_AMELIORER';
    else status = 'CRITIQUE';
  ...
  // ==================== BENCHMARK MULTI-SOCIETES ====================

  async getBenchmarkSocietes(filter: PeriodeFilterDto) {
    // Recuperer les KPIs de la derniere periode pour toutes les societes
    const latest = await this.getLatestKpis();
    ...
    // Calculer les moyennes globales pour reference
    ...
    // Analyser chaque societe
    const societes = latest.map((kpi: any) => {
    ...
    // Trier par score global et attribuer les rangs
    societes.sort((a, b) => b.score_global - a.score_global);
    societes.forEach((s, idx) => {
      s.rang = idx + 1;
    });

    // Identifier le meilleur et le moins bon
