...
export class CommercialService {
  ...
  // ==================== SPI/CPI PERFORMANCE INDEX (INDUSTRIE BTP) ====================

  async getSpiCpiAnalysis(filter: PeriodeFilterDto) {
    const queryBuilder = this.caAffaireRepository
      .createQueryBuilder('caa')
      .leftJoin(DimAffaire, 'a', 'caa.affaire_sk = a.affaire_sk AND a.is_current = true')
      .leftJoin(DimClient, 'c', 'caa.client_sk = c.client_sk AND c.is_current = true')
    ...
    const analyses = affaires.map((a) => {
      ...
      // SPI = Earned Value / Planned Value (basé sur avancement)
      ...
      const actualCost = coutReel > 0 ? coutReel : (heuresRealisees * 50); // 50€/h par défaut
      const cpi = actualCost > 0 ? earnedValueCost / actualCost : 1;

      // Estimation à Achèvement (EAC)
      ...
      // Statut SPI/CPI
      let statusSpi: string;
      if (spi >= 1.1) statusSpi = 'AVANCE';
      else if (spi >= 0.95) statusSpi = 'CONFORME';
      else if (spi >= 0.8) statusSpi = 'RETARD_LEGER';
      else statusSpi = 'RETARD_CRITIQUE';

      let statusCpi: string;
      if (cpi >= 1.1) statusCpi = 'SOUS_BUDGET';
      else if (cpi >= 0.95) statusCpi = 'CONFORME';
      else if (cpi >= 0.8) statusCpi = 'DEPASSEMENT_LEGER';
      else statusCpi = 'DEPASSEMENT_CRITIQUE';

      // Score global performance (0-100)
      const scorePerformance = Math.round(
        Math.min(100, Math.max(0, ((spi + cpi) / 2) * 50))
      );
    ...
    // Tri par score de performance (les moins performantes en premier)
    ...
    let statusGlobal: string;
    if (spiMoyen >= 0.95 && cpiMoyen >= 0.95) statusGlobal = 'PERFORMANT';
    else if (spiMoyen >= 0.85 && cpiMoyen >= 0.85) statusGlobal = 'ACCEPTABLE';
    else if (spiMoyen >= 0.75 || cpiMoyen >= 0.75) statusGlobal = 'A_SURVEILLER';
    else statusGlobal = 'CRITIQUE';
    ...
    return {
      ...
      affaires: analyses.slice(0, 20), // Top 20 à risque
    };
  }

  // ==================== SCORE SATISFACTION CLIENT ====================

  async getClientSatisfactionScore(filter: PeriodeFilterDto) {
    const queryBuilder = this.caClientRepository
      .createQueryBuilder('cac')
      .leftJoin(DimClient, 'c', 'cac.client_sk = c.client_sk AND c.is_current = true')
      .select([
    ...
    const analyses = clients.map((c) => {
      ...
      // === CALCUL SCORE SATISFACTION ===

      // 1. Score Fidélité (30%) - Basé sur récurrence et historique
      const scoreFideliteNorm = Math.min(100, scoreFidelite);

      // 2. Score Paiement (25%) - Pénalités pour retards et impayés
      let scorePaiement = 100;
      if (retardPaiement > 90) scorePaiement -= 50;
      else if (retardPaiement > 60) scorePaiement -= 30;
      else if (retardPaiement > 30) scorePaiement -= 15;
      else if (retardPaiement > 15) scorePaiement -= 5;

      if (nbImpayes > 5) scorePaiement -= 30;
      else if (nbImpayes > 2) scorePaiement -= 15;
      else if (nbImpayes > 0) scorePaiement -= 5;
      scorePaiement = Math.max(0, scorePaiement);

      // 3. Score Croissance (25%) - Variation CA année sur année
      let scoreCroissance = 50;
      if (variationCa > 20) scoreCroissance = 100;
      else if (variationCa > 10) scoreCroissance = 85;
      else if (variationCa > 0) scoreCroissance = 70;
      else if (variationCa > -10) scoreCroissance = 50;
      else if (variationCa > -20) scoreCroissance = 30;
      else scoreCroissance = 10;

      // 4. Score Engagement (20%) - Nombre d'affaires et factures
      let scoreEngagement = 50;
      if (nbAffaires >= 5) scoreEngagement = 100;
      else if (nbAffaires >= 3) scoreEngagement = 80;
      else if (nbAffaires >= 2) scoreEngagement = 60;
      else if (nbAffaires >= 1) scoreEngagement = 40;
      else scoreEngagement = 20;

      // Malus pour avoirs (litiges potentiels)
      const tauxAvoir = nbFactures > 0 ? (nbAvoirs / nbFactures) * 100 : 0;
      if (tauxAvoir > 20) scoreEngagement -= 20;
      else if (tauxAvoir > 10) scoreEngagement -= 10;

      // Score global pondéré
      const scoreGlobal = Math.round(
        scoreFideliteNorm * 0.30 +
        scorePaiement * 0.25 +
      ...
      // Statut satisfaction
      let status: string;
      if (scoreGlobal >= 80) status = 'TRES_SATISFAIT';
      else if (scoreGlobal >= 60) status = 'SATISFAIT';
      else if (scoreGlobal >= 40) status = 'NEUTRE';
      else if (scoreGlobal >= 20) status = 'A_RISQUE';
      else status = 'CRITIQUE';

      // Risque churn
      let risqueChurn: string;
      if (scoreGlobal >= 70 && variationCa >= 0) risqueChurn = 'FAIBLE';
      else if (scoreGlobal >= 50 || variationCa > -10) risqueChurn = 'MODERE';
      else risqueChurn = 'ELEVE';

      // Recommandations
      const recommandations: string[] = [];
      if (retardPaiement > 30) recommandations.push('Relancer les paiements en retard');
      if (variationCa < -10) recommandations.push('Action commerciale prioritaire');
      if (nbImpayes > 2) recommandations.push('Réviser conditions de paiement');
      if (tauxAvoir > 10) recommandations.push('Analyser causes des litiges');
      if (nbAffaires === 0 && caNMoins1 > 0) recommandations.push('Client inactif - relancer');
    ...
    // Tri par score (les moins satisfaits en premier pour action)
    ...
    return {
      ...
      clients_prioritaires: analyses.slice(0, 10), // Top 10 à risque
      clients_fideles: analyses.slice(-10).reverse(), // Top 10 fidèles
    };
  }

  // ==================== SYSTÈME ALERTES PRÉCOCES AFFAIRES ====================

  async getEarlyWarningAffaires(filter: PeriodeFilterDto) {
    const queryBuilder = this.caAffaireRepository
      .createQueryBuilder('caa')
      .leftJoin(DimAffaire, 'a', 'caa.affaire_sk = a.affaire_sk AND a.is_current = true')
      .leftJoin(DimClient, 'c', 'caa.client_sk = c.client_sk AND c.is_current = true')
    ...
    affaires.forEach((a) => {
      ...
      const alertesAffaire: any[] = [];
      let scoreRisque = 0;

      // 1. ALERTE RETARD PLANNING
      const dateDebut = a.date_debut ? new Date(a.date_debut) : null;
      const dateFin = a.date_fin_prevue ? new Date(a.date_fin_prevue) : null;
      ...
      // 5. ALERTE ÉCART FACTURATION/TRAVAUX
      ...
      if (scoreRisque >= 50) niveauRisque = 'CRITIQUE';
      else if (scoreRisque >= 30) niveauRisque = 'ELEVE';
      else if (scoreRisque >= 15) niveauRisque = 'MODERE';
      else niveauRisque = 'FAIBLE';

      // N'ajouter que les affaires avec alertes
      if (alertesAffaire.length > 0) {
        alertes.push({
          ...
          alertes: alertesAffaire.sort((x, y) =>
            (y.severite === 'CRITIQUE' ? 1 : 0) - (x.severite === 'CRITIQUE' ? 1 : 0)
          ),
          indicateurs: {
    ...
    // Tri par score de risque (les plus à risque en premier)
    ...
    return {
      ...
      types_alertes: {
        retard_planning: alertes.filter((a) =>
          a.alertes.some((al: any) => al.type === 'RETARD_PLANNING')).length,
        depassement_budget: alertes.filter((a) =>
          a.alertes.some((al: any) => al.type === 'DEPASSEMENT_BUDGET')).length,
        erosion_marge: alertes.filter((a) =>
          a.alertes.some((al: any) => al.type === 'EROSION_MARGE')).length,
        deadline_proche: alertes.filter((a) =>
          a.alertes.some((al: any) => al.type === 'DEADLINE_PROCHE')).length,
      },
    };
  }
