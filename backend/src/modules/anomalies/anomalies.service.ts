...
export class AnomaliesService {
  ...
  /**
   * Détection de patterns d'anomalies récurrentes
   * Identifie les tendances et corrélations entre anomalies
   */
  async getAnomalyPatterns(filter: PeriodeFilterDto) {
    const anomalies = await this.getAllAnomalies(filter);
    ...
    // Analyser les patterns temporels (si on avait des données historiques)
    // Pour l'instant, on analyse la distribution actuelle
    ...
    // Pattern 1: Concentration d'anomalies client
    ...
    // Pattern 3: Affaires à problèmes multiples
    ...
    const affairesMultiProblemes = affairesUniques.filter(
      (entite) => anomaliesAffaires.filter((a) => a.entite === entite).length > 1
    );
    ...
    // Pattern 4: Problèmes RH liés à la sous-occupation
    ...
    // Pattern 5: Alertes stock récurrentes
    ...
    // Pattern 6: Risque churn concentré
  ...
  /**
   * Génère une heatmap de risque multi-dimensionnelle
   * Permet de visualiser les zones de risque par catégorie et sévérité
   */
  async getRiskHeatmap(filter: PeriodeFilterDto) {
    const anomalies = await this.getAllAnomalies(filter);

    // Définir les catégories et sous-catégories
    const categories = ['Affaires', 'Clients', 'Stock', 'RH'];
    const severites: Anomalie['severite'][] = ['CRITIQUE', 'HAUTE', 'MOYENNE', 'BASSE'];

    // Construire la matrice de heatmap
    ...
    categories.forEach((categorie) => {
      severites.forEach((severite) => {
        const matching = anomalies.filter(
          (a) => a.categorie === categorie && a.severite === severite
        );

        // Score de risque: pondéré par sévérité
    ...
    const scoresParCategorie = categories.map((categorie) => {
      ...
      let niveau: 'CRITIQUE' | 'ELEVE' | 'MODERE' | 'FAIBLE';
      if (totalScore >= 20) niveau = 'CRITIQUE';
      else if (totalScore >= 10) niveau = 'ELEVE';
      else if (totalScore >= 5) niveau = 'MODERE';
      else niveau = 'FAIBLE';
    ...
    let niveauGlobal: 'CRITIQUE' | 'ELEVE' | 'MODERE' | 'FAIBLE';
    if (scoreGlobal >= 50) niveauGlobal = 'CRITIQUE';
    else if (scoreGlobal >= 30) niveauGlobal = 'ELEVE';
    else if (scoreGlobal >= 15) niveauGlobal = 'MODERE';
    else niveauGlobal = 'FAIBLE';

    // Top risques à adresser
    ...
    // Recommandations par catégorie à risque
  ...
  /**
   * Analyse des tendances d'anomalies (évolution dans le temps)
   * Nécessite des données historiques pour être vraiment pertinent
   */
  async getAnomalyTrends(filter: PeriodeFilterDto) {
    // Pour une analyse de tendance complète, on aurait besoin de données historiques
    // Ici on simule avec les données actuelles en les projetant
    ...
    // Zone 1: Taux d'anomalies critiques
    ...
    // Zone 2: Concentration par catégorie
    const maxCategorie = Math.max(
      synthese.par_categorie.affaires,
      synthese.par_categorie.clients,
      synthese.par_categorie.stock
    );
    ...
    const actionsPrioritaires: Array<{
      priorite: number;
      categorie: string;
      action: string;
